...
This `Vignette` presents the methodology that allows to import Epidemium data and merge them. It is assumed following [Epidemium data](http://qa.epidemium.cc/data/epidemiology_dataset/) are **stored in the working directory**:
...
# Importing separately dataframes

`csv` files can be imported by the use of the `import_*` functions in the `OpenCancer` package. Data can be imported from an url or from a file stored in the user computer.

In this vignette, we will assume data are stored in the machine, in a subdirectory `./inst`. The files used to create this vignette can thus be found in the `./vignettes/inst` directory of the package. 
...
The import functions share the same structure. The parameters `colstokeep` allow to only keep a few variables. If `colstokeep = NULL`, all variables are kept in memory. We will keep only first 100 variables in each dataframe
...
We have chosen to use `training_IARC` data because they exhibit fewer missing values for the year variable, one of the identifiers we must use. 
...
To ensure data will be consistently merged, we import the `creation zonier.xls` file. All countries in the FAO, ILO and World Bank database should thus be assigned a unique and consistent key, a necessary step before merging dataframes.

```{r, message=FALSE,warning=FALSE}

link <- "https://github.com/linogaliana/OpenCancer/raw/master/vignettes/inst/creation%20zonier.xls"
...
Using the `codes` dataframe, we use the `matchkeys` function to match countries in a dataframe (e.g. `df_FAO`) with its identifiers. Default source is FAO. 
...
Now that all explanatory variables dataframes are consistent, we can merge them. 
...
To ensure consistency with the training data, we control the countries that do not match with the keys list. Since the number is limited (8), we can make a manual change.
...
To limit the amount of RAM required by R, we clean the working directory by removing dataframes that are no longer necessary. We also fix as a starting point 1970 where we have all covariates dataframes that begin. 
...
Let's say we want to convert each variable with cardinal lower than 12 as `factor`, in that case we must put `check.levels = F`
...
A first useful function has been designed to identify potential miscoding issues. `convert_fakezero` has been designed to inspect variables where zeros might rather be `NAs`. Rather than printing variables code that are obscure, we use our dataframes storing variables labels (`df_coding`)
...
The dataframe is now ready for statistical modelling. When complete, i.e. when putting `colstokeep = NULL` rather than `colstokeep = 1:20` as above, it might be preferrable to use `big.matrix` object (see `bigmemory` documentation) rather than standard dataframes. 
